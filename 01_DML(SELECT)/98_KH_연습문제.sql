...
SELECT *
FROM EMPLOYEE
WHERE ABS(FLOOR((HIRE_DATE - SYSDATE))) >= 7300;
...
SELECT EMP_NAME, DEPT_CODE, SUBSTR(EMP_NO, 1,2) || '년' AS "년",  SUBSTR(EMP_NO, 2,1) || '월' AS "월",
SUBSTR(EMP_NO, 3,1) || '일' "일", 
EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM (TO_DATE(SUBSTR(EMP_NO, 1,6), 'RRMMDD'))) AS "만 나이"
FROM EMPLOYEE;
...
-- 주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR(EMP_NO, 1,6) AS "앞자리", SUBSTR(EMP_NO, 8,14) AS "뒷자리",  SUBSTR(EMP_NO, 1,6) + SUBSTR(EMP_NO, 8,14) AS "합"
FROM EMPLOYEE
WHERE EMP_ID = 201;
...
DECODE(SUBSTR(EXTRACT(YEAR FROM (HIRE_DATE)), 1,4), 2001, EXTRACT(YEAR FROM (HIRE_DATE)))
FROM EMPLOYEE
